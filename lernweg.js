'use strict';
const tasks=window.HOLZ_KURS.tasks,KEY='hbg_holz_navigator_v2';
const symbols={mappe:'📁 MAPPE',ipad:'📱 iPAD',lesen:'📖 LESEN',erkunden:'🔬 ERKUNDEN',halt:'🛑 HALTEPUNKT'};
let state={task:'A1',index:0,phase:'step',done:[],opened:{},helps:{},extras:[],introDone:false};
try{const v=JSON.parse(localStorage.getItem(KEY));if(v&&typeof v==='object'){const t=tasks.find(t=>t.id===v.task);if(t){state.task=t.id;state.index=Number.isInteger(v.index)?Math.max(0,Math.min(v.index,t.steps.length)):0;}if(['step','finished','extra'].includes(v.phase))state.phase=v.phase;for(const key of ['done','extras'])if(Array.isArray(v[key]))state[key]=v[key].filter(id=>tasks.some(t=>t.id===id));for(const key of ['opened','helps'])if(v[key]&&typeof v[key]==='object'&&!Array.isArray(v[key]))state[key]=v[key];state.introDone=v.introDone===true;}}catch{}
const query=new URLSearchParams(location.search),requested=tasks.find(t=>t.id===query.get('auftrag'));
